...
// =============================================================
// KONFIGURASI (AMAN - Dibaca dari Environment Variables)
// =============================================================

// Ambil variabel dari hosting, atau gunakan nilai default (untuk RCON_HOST & PORT)
// Anda HARUS mengatur ini di "Variables" hosting Anda
const RCON_HOST = process.env.RCON_HOST;
const RCON_PORT = process.env.RCON_PORT;

// Ambil port dari hosting, atau gunakan 3000 jika tes lokal
const NODE_PORT = process.env.PORT || 3000; // WAJIB PAKAI process.env.PORT

// Ambil variabel rahasia dari hosting
// Ini WAJIB Anda atur di "Variables" pada dashboard hosting Anda
...
if (!RCON_PASSWORD || !SOCIABUZZ_WEBHOOK_TOKEN || !RCON_HOST || !RCON_PORT) {
  ...
  console.error("di 'Variables' (Environment Variables) hosting Anda!");
  console.error("================================================================");
}
...
app.post("/webhook/sociabuzz", verifySociabuzzToken, (req, res) => {
  ...
  if (amount >= 200000) {
    ...
    // ⚠️ Ganti 0 150 0 dengan koordinat spawn Anda di "Variables" RCON_SPAWN_COORDS
    const spawnCoords = process.env.RCON_SPAWN_COORDS || "0 150 0";
    minecraftCommand = `tp @a ${spawnCoords}`; 
